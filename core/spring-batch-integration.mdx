...
The default implementation reads from `FineractProperties` (`fineract.partitioned.<jobName>.*`) so each job can be tuned independently from `application.properties` or environment variables. Operators tuning COB throughput will end up here:
...
```properties
# Per-job overrides (illustrative)
fineract.partitioned.LOAN_COB.partition-size=500
fineract.partitioned.LOAN_COB.chunk-size=100
fineract.partitioned.LOAN_COB.retry-limit=3
fineract.partitioned.LOAN_COB.thread-pool.core-pool-size=4
fineract.partitioned.LOAN_COB.thread-pool.max-pool-size=8
fineract.partitioned.LOAN_COB.thread-pool.queue-capacity=200
fineract.partitioned.LOAN_COB.poll-interval=1000
```
