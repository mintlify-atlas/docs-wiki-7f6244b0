...
- **basicauth.enabled** wires `TenantAwareBasicAuthenticationFilter` (backed by `AuthTenantDetailsService`) into the Spring Security filter chain. Username and password are matched against `m_appuser`.
