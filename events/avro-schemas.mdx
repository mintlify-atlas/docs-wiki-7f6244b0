...
The Java classes the schemas generate land in `fineract-avro-schemas/build/generated/java/src/main/java/org/apache/fineract/avro` after a Gradle build (the hand-written `src/main/java/org/apache/fineract/avro/generator` package only contains the preprocessor that resolves the BigDecimal template before generation).
...
3. **Generated classes only.** Hand-edited code under `fineract-avro-schemas/build/generated/…` is not allowed — the Gradle Avro plugin (`com.github.davidmc24.gradle.plugin.avro-base`) regenerates everything from the `.avsc` files at build time. The only hand-written code in the module is the schema preprocessor under `src/main/java/org/apache/fineract/avro/generator`.
