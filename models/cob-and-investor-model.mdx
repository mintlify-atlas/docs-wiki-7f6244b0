...
description: "m_loan_account_lock, m_savings_account_lock, m_wc_loan_account_lock, m_external_asset_owner, m_external_asset_owner_transfer."
---
...
erDiagram
    ...
    m_loan ||--o| m_wc_loan_account_locks : "loan_id"
...
## m_savings_account_locks — savings counterpart

Same shape as the loan lock table but for savings accounts. Added so the same COB serialisation pattern can be applied to savings interest posting / dormancy jobs.

| Column | Type | Notes |
| --- | --- | --- |
| `savings_id` | `BIGINT PK` → `m_savings_account.id` | One-to-one with account. |
| `lock_owner` | `VARCHAR(100) NOT NULL` | `SAVINGS_COB_CHUNK_PROCESSING_WORKER` or `SAVINGS_INLINE_COB_PROCESSING`. |
| `lock_placed_on` | `TIMESTAMP NOT NULL` | Acquisition time. |
| `lock_placed_on_cob_business_date` | `DATE` | COB business date. |
| `error` | `VARCHAR(255)` | Error message. |
| `stacktrace` | `TEXT` | Full trace. |
| `version` | `BIGINT NOT NULL` | Optimistic lock. |

Foreign key on `savings_id` → `m_savings_account(id)`.

## m_wc_loan_account_locks — working-capital loan lock

Working-capital loans (per the `fineract-working-capital-loan` module) have their own dedicated lock table to keep their COB pipeline isolated from regular loans.

| Column | Type | Notes |
| --- | --- | --- |
| `loan_id` | `BIGINT PK` → `m_loan.id` | One-to-one with working-capital loan. |
| `lock_owner` | `VARCHAR(100) NOT NULL` | Worker identifier. |
| `lock_placed_on` | `TIMESTAMP NOT NULL` | Acquisition time. |
| `lock_placed_on_cob_business_date` | `DATE` | COB business date. |
| `error` | `VARCHAR(255)` | Error message. |
| `stacktrace` | `TEXT` | Full trace. |
...
Foreign key on `loan_id` → `m_loan(id)`.
...
| `m_savings_account_locks` | `savings_id` | `m_savings_account(id)` |
| `m_wc_loan_account_locks` | `loan_id` | `m_loan(id)` |
...
| `m_savings_account_locks` | PK on `savings_id` | Lock check. |
| `m_wc_loan_account_locks` | PK on `loan_id` | Lock check. |
