...
description: "m_scheduler_job, m_scheduler_job_history, BATCH_JOB_EXECUTION, BATCH_STEP_EXECUTION, BATCH_JOB_EXECUTION_PARAMS — scheduler and Spring Batch metadata."
---
...
1. A bespoke scheduler defined by `m_scheduler_job` and `m_scheduler_job_history`, which holds the cron expression, last-run audit, and lock fields per job.
...
The Fineract-native tables ship in `fineract-provider/src/main/resources/db/changelog/tenant/parts/0001_initial_schema.xml`; the Spring Batch tables come from Spring's own `schema-mysql.sql` and `schema-postgresql.sql` and are wired in through `JobConfig`. This page documents both clusters so you can correlate a row in `m_scheduler_job` with its corresponding `BATCH_JOB_EXECUTION` row.

## Entity-relationship overview

```mermaid
erDiagram
    m_scheduler_job ||--o{ m_scheduler_job_history : "job_id"
    m_scheduler_job ||--o{ job_parameters : "scheduler_job_id"
    m_scheduler_job }o--o| job_run_history : "via job_id"
    ...
    m_scheduler_job_run_history }o--|| m_scheduler_job : "job_id"
```

## m_scheduler_job — the job registry
...
| --- | --- | --- |
| `id` | `BIGINT PK auto` | Surrogate. |
| `job_id` | `BIGINT NOT NULL` → `m_scheduler_job.id` | Owning job. |
| `parameter_name` | `VARCHAR(50)` | Key. |
| `parameter_value` | `VARCHAR(50)` | Value. |

## m_scheduler_job_run_history — audit
...
Foreign key: `FK_m_scheduler_job_run_history_job` → `m_scheduler_job(id)` cascade delete. Index on `(job_id, version desc)` for the API's "latest 10 runs" query.

## m_scheduler_group — concurrency groups

A small table that lists the group ids used by `m_scheduler_job.scheduler_group` and the named lock each group serialises on.
...
| Child | Column | Parent |
| --- | --- | --- |
| `m_scheduler_job_run_history` | `job_id` | `m_scheduler_job(id)` |
| `m_scheduler_job_parameters` | `job_id` | `m_scheduler_job(id)` |
...
| Table | Index | Use |
| --- | --- | --- |
| `m_scheduler_job` | `name` | Lookup. |
| `m_scheduler_job` | `(node_id, is_active)` | Clustered scan. |
| `m_scheduler_job_run_history` | `(job_id, version desc)` | Recent runs. |
...
## Correlating m_scheduler_job_run_history and BATCH_JOB_EXECUTION

A single COB run produces:

- One row in `m_scheduler_job_run_history` with `job_id = <m_scheduler_job.id for COB>`, `trigger_type`, `start_time`, `end_time`, `status`.
...
The cross-reference between the two systems is by wall-clock time: there is no shared id. Operations dashboards typically join `m_scheduler_job_run_history.start_time` with `BATCH_JOB_EXECUTION.START_TIME` within a small window.
...
WHERE  job_id = (SELECT id FROM m_scheduler_job WHERE name = 'Loan COB')
...
When multiple Apache Fineract nodes share a tenant database, the `node_id` column on `m_scheduler_job` pins each job to a specific node. The scheduler on each node periodically scans `m_scheduler_job` filtered by `node_id` and ignores jobs that belong to a sibling.
...
If the scheduler is paused (`m_scheduler_detail.is_suspended = true`) or the node is offline when a trigger should fire, the job is marked as misfired. On startup, if `m_scheduler_detail.execute_misfired_jobs = true`, the scheduler will run misfired jobs once on resume.

`m_scheduler_job.is_misfired` reflects the most recent misfire state for the row, and the operational UI surfaces it on the jobs dashboard so ops can decide whether to force-run.

The relationship between the two clusters is loose but real: when the Fineract scheduler trigger fires a Spring Batch job, `m_scheduler_job_run_history` and `BATCH_JOB_EXECUTION` will both gain one row for the same wall-clock execution. The Fineract row is what the admin UI shows under "Recent runs", and the Spring Batch rows are what the developer reaches for when they need step-level chunk counts, skip stats, or restart context.
