...
description: "m_portfolio_command_source, m_external_event, m_external_event_configuration — the command audit and event outbox."
---

Every write into Apache Fineract is a command — a JSON envelope that flows through the command framework, gets routed to a handler, and produces a deterministic side-effect. Two table clusters back that pipeline. The first records the command itself and its maker-checker lifecycle in `m_portfolio_command_source`. The second is the external-event outbox in `m_external_event` and `m_external_event_configuration`, which captures the business events emitted by command handlers and replays them to downstream subscribers.
...
erDiagram
    ...
    m_external_event ||--o{ m_external_event : "idempotency_key"
```
...
| Column | Type | Notes |
| --- | --- | --- |
| `type` | `VARCHAR(100) PK` | Event type name (matches `m_external_event.type`). |
...
## On historical audit tables

Older Fineract codebases referenced separate `m_audit_log` / `m_audit` / `m_audit_query` tables for generic auditing. Upstream Apache Fineract has consolidated on `m_portfolio_command_source` as the single source of truth for both the command audit and the maker-checker lifecycle — there are no standalone `m_audit*` tables shipped in the current changelog. The audit search APIs and the maker-checker inbox both read directly from `m_portfolio_command_source`.
...
| `m_external_event` | (logical) `type` | `m_external_event_configuration(type)` |
...
| `m_external_event` | `(status, created_at)` | Publisher polling. |
| `m_external_event` | `(type, created_at)` | Per-type replay. |
| `m_external_event` | `(aggregate_root_id, type)` | Entity replay. |
...
This makes the table the durable backing store for at-least-once REST semantics — the same guarantee Apache Fineract gives Kafka consumers and webhook subscribers through `m_external_event`.
...
- Multiple rows in `m_external_event` — at minimum `LoanDisbursalBusinessEvent` and `LoanDisbursalTransactionBusinessEvent`, with `aggregate_root_id = <loan id>` and `status = READY_TO_BE_SENT`.
...
- `m_external_event` rows with `status = SENT` and `sent_at < now() - retention window` can be deleted by a purge job. The platform ships no purge job out of the box; operations teams typically define one with a small SQL window.
...
The categories on `m_external_event` group the type catalogue into roughly the following buckets, which match the modules that emit them:
...
The command and audit tables therefore close the loop on every state-changing write: a single row in `m_portfolio_command_source` carries the request, its maker-checker state, and the result envelope; matching rows in `m_external_event` describe the downstream business events that command produced; together they provide the deterministic audit trail and the integration contract for everything that ever happened on the tenant.
